...
int eval_total_num = 0;
int eval_yolo_success_num = 0;
int eval_tracker1_success_num = 0;
int eval_tracker2_success_num = 0;
int eval_tracker3_success_num = 0;
    ...
    while (1){
        ...
        if ((start_2 == 1)){
            ...
            if (pic_is_ready_2==1){
                ...
                if (ok){
                    eval_tracker2_success_num+=1;
                }
                output_bbox = tracker2_bbox;
...
int main() {
    ...
    frame = cv::imread("/home/chenyl/CLionProjects/untitled1/images/396.jpg");
...
//    th5.detach();
    ...
    while (1){
        ...
        cv::Mat frame2 = cv::imread("/home/chenyl/CLionProjects/untitled1/images/"+std::to_string(total_num)+".jpg");
        frame = frame2.clone();
        total_num+=1;
        finished = 0;
        ...
        if (start == 1){
            ...
            if (ok){
                eval_tracker1_success_num+=1;
//                output_bbox = cv::Rect2d (bbox.x, bbox.y, bbox.width, bbox.height);
            }

//            if ((bbox.width!=0)&&(tracker2_bbox.width!=0)){
//                result_bbox = bbox;
//                eval_tracker3_success_num+=1;
////                result_bbox = cv::Rect(cv::Point((bbox.x+tracker2_bbox.x)/2, (bbox.y+tracker2_bbox.y)/2), cv::Point((bbox.x+bbox.width+tracker2_bbox.x+tracker2_bbox.width)/2, (bbox.y+tracker2_bbox.y+bbox.height+tracker2_bbox.height)/2));
//            }
//            else{
//                if (bbox.width==0){
...
//        writeCvRectToFile(result_bbox, ("/home/chenyl/CLionProjects/untitled1/results_exp4-try/"+std::to_string(total_num)+".yaml").data());
    }
    capture.release();
    std::cout<<"yolo_num:"<<eval_yolo_success_num<<std::endl;
    std::cout<<"tracker1_num:"<<eval_tracker1_success_num<<std::endl;
    std::cout<<"tracker2_num:"<<eval_tracker2_success_num<<std::endl;
    std::cout<<"tracker3_num:"<<eval_tracker3_success_num<<std::endl;
//    writer.release();
    return 0;
}
